...
function generateCollapseNodeString(navNode, treeNode) {
  return "<a class=\"plainlink\" style=\"border-width: 2px; border-style: outset; padding-left: 1px; padding-right: 2px; background-color: blue; color: white;\" href=\"javascript:void(0);\" onClick=\"collapseTreeNode(\'" + navNode.id + "\',\'" + treeNode.id + "\');\">-</a>"
...
function expandTreeHierarchy(treeNodeId) {
  ...
  if (null != expandnode) {
    for (expandnode = getNavParentForNode(expandnode); null != expandnode;
...
function addNavigationToTree(nodeId, selectedNodeId,checkBrowserArgs) {
  ...
  if (arguments.length > 2) {
    var requestArgList = top.document.URL.split("?");
    if (requestArgList.length > 1) {
      var passedNodeId = "";
      requestArgList = requestArgList[1].split("&");
      for (i = 0; i < requestArgList.length; i++) {
