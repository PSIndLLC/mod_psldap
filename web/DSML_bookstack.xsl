  ...
  <html>
	...
	<tr>
	  <td width="256px" valign="top">
	    <xsl:call-template name="libraryMgr" />
	  </td>
	  <td>
	    <table id='cardTable'>
	      <tr class="resultRow">
...
<xsl:template name="libraryMgr" >
  ...
  <p><a href="<xsl:value-of select="$v_baseURI" />/">Contact directory</a></p>
  <p><a href="javascript:void loadTemplateRecord('<xsl:value-of select="$v_baseURI" />/DSML_new_v_o.xml', '<xsl:value-of select="$v_baseURI" />/DSML_editform.xsl', null, '&amp;dn=' + encodeURIComponent(document.queryForm.dn.value));">Add a new vendor</a></p>
  Lookup contacts in family address book:<br />
  <form id="queryForm" name="queryForm" onsubmit="return generateQueryString();" method="POST" action="<xsl:value-of select="$v_baseURI" />/ldapupdate" target="_new">
    <table width="100%">
      <tr>
	<td align="left">
	  ...
	  <input type="hidden" name="xsl1" size="32" maxlength="32" value="<xsl:value-of select="$v_baseURI" />/DSML_cards.xsl" />
