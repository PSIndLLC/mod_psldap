...
### Authorization
1.  Supports authorization based solely on authenticated status
2.  Based on group membership
3.  Based on attribute value on the user record
...
- Allow an authenticated web user to query the LDAP server for information, within the limits of their privileges on the server
- Override client side XSL rendering of an XML response to force rendering an HTML response on the server
- Allow the user to edit any attributes in his/her own record, given he/she is granted read/write access to those attributes by the LDAP server. This feature assumes the attribute name in the LDAP server is used as the value name in a POST operation.
- Allow the LDAP administrator to edit any record in the LDAP server
- Support configuration of access restriction to allow only https connections
- Drag and drop movement of records from one node to another in the directory tree - accomodating organizational restructuring or relocation without recreating records
